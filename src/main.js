...
if(FILMS_COUNT > FILM_COUNT_PER_STEP){
  ...
  showMoreButton.addEventListener('click', (evt) => {
    ...
    films
      ...
      .forEach((film) => render(siteContentFilmsListContainer, createFilmCardTemplate(film), 'beforeend'));
...
let topRated = films.map((it) => (it.raiting)).sort().slice(-FILM_EXTRA_COUNT);
console.log(topRated);
let mostCommented = films.map((it) => (it.comments).length).sort().slice(-FILM_EXTRA_COUNT);
console.log(mostCommented);

for(let i=0; i<FILM_EXTRA_COUNT; i++){

  // получить 2 самых коментируемых + 2 самых оцененных, потом вывести их тут
...
render(footerStatistic, createFooterStatisticTemplate(films.length), 'beforeend');
